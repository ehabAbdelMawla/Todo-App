...
export default function BranchBox({ id, complementRate, branchName, history }) {
        ...
        swal("Write New Name:", {
            ...
            .then((value) => {
                if (value != null && value.trim() != "") {
                    Firebase.database().ref().child(`users/${user.uid}/branches/${id}`).update({ 'branchName': aes256.encrypt(App.key, value) }).then(() => {
    ...
    return (
        <div className={`branchCard wow  ${animateClass}`} ref={referance}
            onAnimationEnd={({ animationName }) => {
                if (animationName === "fadeOutDown") { dispatch({ type: "REMOVE_BRANCH", id }) }
            }}>
            <div className="head" >
                <button type="button" onClick={updateBranch} >
                    < i className="fa fa-edit" ></i></button>
                <button type="button" onClick={deleteBranch} >
                    < i className="fa fa-trash" > </i>
                </button>
            </div>
            <div className="contextBody" onClick={openToDoOfbranch} >
                <CircularProgressbar
                    value={complementRate ? complementRate : 0}
                    maxValue={100}
